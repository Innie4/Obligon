  ...
  return (
    <ModalShell onClose={onClose} label="Edit staff permissions">
      <form onSubmit={submit}>
        ...
        <div className="max-h-[calc(92vh-160px)] overflow-y-auto bg-[#f7f7fd] px-6 py-8"><h3 className="font-display text-xl font-extrabold text-obligon-blue">Edit Staff Permissions</h3><p className="mt-1 text-sm text-obligon-text">Define module-specific access levels for this staff member across the Obligon LTD ecosystem.</p>
...
function ProvisionFleetModal({ onClose }: { onClose: () => void }) {
  ...
  return (
    <ModalShell onClose={onClose} label="Provision new fleet"><form onSubmit={submit} className="max-h-[92vh] overflow-y-auto bg-[#f7f7fd] p-8"><div className="flex items-start justify-between gap-4"><div><p className="font-display text-2xl font-extrabold">Obligon LTD<br />Admin</p><p className="mt-1 text-[10px] font-extrabold uppercase text-obligon-text">INTERNAL VIEW</p></div><button className="grid size-9 place-items-center rounded-lg text-obligon-text focus:outline-none focus:ring-2 focus:ring-obligon-green" onClick={onClose} type="button" aria-label="Close fleet modal"><X size={22} /></button></div><h2 className="mt-8 font-display text-3xl font-extrabold">Provision New Fleet</h2><p className="mt-2 text-sm text-obligon-text">Onboard a new enterprise logistics partner to the Obligon LTD network.</p>
