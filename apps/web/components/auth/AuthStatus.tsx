  ...
  return (
    <div className="grid w-full max-w-[960px] items-center gap-10 lg:grid-cols-[480px_1fr]">
      <section className="rounded-3xl border border-obligon-border bg-white p-8 text-center shadow-card">
        ...
        <p className="mx-auto mt-4 max-w-sm text-base leading-6 text-obligon-text">{item.body}</p>

        {status === "progress" ? (
          <div className="mt-8">
            <div className="h-2 overflow-hidden rounded-full bg-[#e6eeff]">
              <div className="h-full w-1/4 rounded-full bg-obligon-green" />
            </div>
            <p className="mt-3 text-xs font-bold uppercase tracking-[1.2px] text-obligon-green">Processing</p>
          </div>
        ) : (
          <div className="mt-8 flex flex-col justify-center gap-3 sm:flex-row">
            <Link href={status === "success" ? routes.dashboard : routes.login} className="inline-flex h-12 items-center justify-center rounded-lg bg-obligon-green px-6 text-base font-bold text-white shadow-green">
              {status === "success" ? "Go to Dashboard" : "Try Again"}
            </Link>
            <Link href={status === "success" ? routes.fuelvista : routes.support} className="inline-flex h-12 items-center justify-center rounded-lg border border-obligon-border px-6 text-base font-bold text-obligon-navy">
      ...
      <aside className="hidden rounded-[32px] bg-obligon-navy p-10 text-white lg:block">
        <div className="flex h-[420px] flex-col justify-end rounded-3xl border border-white/10 bg-[radial-gradient(circle_at_top_right,rgba(170,248,87,0.25),transparent_45%)] p-8">
          ...
          <h2 className="mt-4 font-display text-4xl font-extrabold leading-[44px]">Nigeria&apos;s most trusted energy fintech infrastructure.</h2>
