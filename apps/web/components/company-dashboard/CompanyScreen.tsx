...
function Table({ title, columns, rows, actionLabel = "View", onAction }: { title: string; columns: string[]; rows: Row[]; actionLabel?: string; onAction?: () => void }) {
  return (
    <section className="overflow-hidden rounded-lg border border-[#dfe5ec] bg-white">
      <div className="flex items-center justify-between border-b border-[#dfe5ec] px-6 py-5">
        ...
        <button type="button" onClick={onAction} className="text-sm font-extrabold text-obligon-green">{actionLabel}</button>
      </div>
          ...
          <tbody className="divide-y divide-[#eef2f6]">
            {rows.map((row, index) => (
              <tr key={`${row.cells[0]}-${index}`}>
                ...
                <td className="px-6 py-4 text-right"><button type="button" onClick={onAction} className="font-bold text-obligon-green">•••</button></td>
              </tr>
            ))}
          </tbody>
        </table>
